...
/*
    ...
    속도를 맞추라고 일반적으로 fk에 index를 설정한다.
 */
...
public class Reply extends BaseEntity{
    ...
    /*
        ...
        FetchType.LAZY : 일단은 대체 객체로 채워놀고, 나중에 필요할 때 읽어들인다.
        FetchType.EAGER : Board도 테이블에서 즉시 읽어들인다.

        DBMS에서는 Board가 부모, Reply가 자식인 제약조건이 생성된다.
        (Board테이블의 PK가 Reply테이블의 FK로 전이)

        단방향 연결방식에서는 @OneToMany를 권장하지 않고 @ManyToOne을 권장한다.
        왜냐하면 @OneToMany를 사용하면(Board클래스에 Reply replyㅡㄹㄹ 추가하고 @OneToMany설정)
        Reply에 새로운 데이터를 넣을 때 불필요한 Update과정이 추가로 필요하기 때문에 좀더 부하가 걸린다.
